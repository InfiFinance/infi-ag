...
module.exports.deployMinimalRouter = (networkName) => {
    ...
    const exportEnv = async ({ getNamedAccounts, deployments }) => {
        ...
        console.log('InfiRouter deployment arguments: ', deployArgs)

        const name = 'MinimalInfiRouter'
        const contractName = 'InfiRouter'
...
module.exports.deployInfiWrapRouter = (networkName, routerDeploymentName) => {
    const deployOptions = require('../misc/deployOptions')[networkName]
    const exportEnv = async ({ getNamedAccounts, deployments }) => {
        const { deployer } = await getNamedAccounts()
        if (!deployOptions)
        ...
        console.log('InfiWrapRouter deployment arguments: ', deployArgs)

        const name = 'InfiWrapRouter'
        const contractName = 'InfiWrapRouter'
        const optionalArgs = { gas: 4000000 }
        const deployFn = _deployContract(name, contractName, deployArgs, optionalArgs)
        await deployFn({ getNamedAccounts, deployments })
    }
...
module.exports.deployInfiWrapRouterAlt = (networkName, routerDeploymentName) => {
    const deployOptions = require('../misc/deployOptions')[networkName]
    const exportEnv = async ({ getNamedAccounts, deployments }) => {
        const { deployer } = await getNamedAccounts()
        if (!deployOptions)
        ...
        console.log('InfiWrapRouterAlt deployment arguments: ', deployArgs)

        const name = 'InfiWrapRouter'
        const contractName = 'InfiWrapRouterAlt'
        const optionalArgs = { gas: 4000000 }
        const deployFn = _deployContract(name, contractName, deployArgs, optionalArgs)
        await deployFn({ getNamedAccounts, deployments })
    }
